...
namespace AdvancedKeyBindings.Extensions
{
    public static class FarmAnimalExtension
    {
        ...
        {
            return Game1.getFarm().buildings.Where(building => building.buildingType.Contains(animal.buildingTypeILiveIn) &&
                                                               !((AnimalHouse) building.indoors.Value).isFull()).ToList();
        }
    }
}